  ...
  switch v.(type) {
    case string:
      ...
      if ipv4HexRE.MatchString(v) {
        ...
        ip = net.ParseIP(v)
        if ip == nil { return "", 0, fmt.Errorf("invalid IPv4: failed to parse") }
        ip = ip.To4()
        if ip == nil { return "", 0, fmt.Errorf("invalid IPv4: parsed, but not as IPv4") }
      }
    case []interface{}:
      v := v.([]interface{})
      if len(v) != 4 { return "", 0, fmt.Errorf("invalid IPv4: array length not 4") }
      ip = net.IP{0, 0, 0, 0}
      for i, v := range v {
        switch v.(type) {
          case float64:
  ...
  switch v.(type) {
    case string:
      ...
      if ipv6HexRE.MatchString(v) {
        ...
        ip = net.ParseIP(v)
        if ip == nil { return "", 0, fmt.Errorf("invalid IPv6: failed to parse") }
        ip = ip.To16()
        if ip == nil { return "", 0, fmt.Errorf("invalid IPv6: parsed, but no IPv6") }
      }
    case []interface{}:
      v := v.([]interface{})
      ...
      for i, v := range v {
        switch v.(type) {
          case float64:
            if v.(float64) < 0 { return "", 0, fmt.Errorf("invalid IPv6: part out of range") }
            v := uint64(v.(float64))
            if v > maxVal { return "", 0, fmt.Errorf("invalid IPv6: part out of range") }
            setPart(i, v)
          case string:
            v, err := strconv.ParseUint(v.(string), 0, bitSize)
            if err != nil { return "", 0, fmt.Errorf("invalid IPv6: %s", err) }
            if v > maxVal { return "", 0, fmt.Errorf("invalid IPv6: part out of range") }
            setPart(i, v)
          default:
