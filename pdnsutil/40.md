...
add-zone-key ZONE {zsk|ksk} [BITS] [active|inactive]
...
create-slave-zone ZONE master-ip [master-ip..]
                                   Create slave zone ZONE with master IP address master-ip
...
import-zone-key ZONE FILE          Import from a file a private key, ZSK or KSK
       [active|inactive] [ksk|zsk]  Defaults to KSK and active
...
list-keys [ZONE]                   List DNSSEC keys for ZONE. When ZONE is unset or "all", display all keys for all zones
...
set-publish-cdnskey ZONE           Enable sending CDNSKEY responses for ZONE
set-publish-cds ZONE [DIGESTALGOS] Enable sending CDS responses for ZONE, using DIGESTALGOS as signature algirithms
                                   DIGESTALGOS should be a comma separated list of numbers, is is '1,2' by default
