...
func lookup(params map[string]interface{}) (interface{}, error) {
  ...
  for _, item := range response.Kvs {
    ...
    if len(item.Value) == 0 { return false, fmt.Errorf("empty value") }
    qp := qp // clone
    if qp.isANY() {
    ...
    if item.Value[0] == '{' {
      ...
      switch qp.qtype {
        ...
        // TODO more qtypes
        default: return false, fmt.Errorf("unknown/unimplemented qtype '%s', but have (JSON) object data for it (%s)", qp.qtype, qp.recordKey())
      }
      if err != nil { return false, err }
    } else {
